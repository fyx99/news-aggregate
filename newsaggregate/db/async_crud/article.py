...
async def get_articles_clean(db: AsyncDatabase) -> List[Article]:
    rows = await db.query("""
                        SELECT id, url, amp_url, image_url, title, left(summary, 400) as summary, update_date, feed, title_hash, status, left(text, 600) as text, publisher, icon_url from articles_clean
                        """, 
                        result=True)
